# app.py - FINAL Code with Loader & Corrected Scroll Logic
# --- Imports ---
...
# --- Helper Functions ---
# Using the get_edge_color function provided by the user
def get_edge_color(image_url, edge_width_percent=10):
    ...
    try:
        ...
        if img_width <= 1 or img_height <= 0: return "#808080" # Check added
        width = max(1, int(img.width * (edge_width_percent / 100)))
        if width > img_width: width = img_width # Check added
        edge = img.crop((0, 0, width, img.height))
        stat = ImageStat.Stat(edge)
        avg_color = (128, 128, 128) # Default grey
        if hasattr(stat, 'mean') and stat.mean:
            ...
            if isinstance(avg_color_float, (list, tuple)) and len(avg_color_float) >= 1:
                 avg_color = tuple(int(c) for c in avg_color_float[:3]) # Ensure 3 values
                 if len(avg_color) == 1: avg_color = (avg_color[0],)*3
            elif isinstance(avg_color_float, (int, float)): # Grayscale
                 gray_val = int(avg_color_float); avg_color = (gray_val,)*3
        # Added length check before format
        if len(avg_color) < 3: avg_color = (avg_color[0],)*3 if len(avg_color)>0 else (128,)*3
        hex_color = "#{:02x}{:02x}{:02x}".format(*avg_color[:3])
        return hex_color
    except Exception as e:
        print(f"Warn: Img process fail {image_url.split('/')[-1]} {e}")
        return "#808080"

# Using the get_books_from_shelf function provided by the user
def get_books_from_shelf(url):
    ...
    try:
        page = 1
        # Simplified total books estimation - can be refined if needed
        ...
        if total_books == 0: total_books = 1 # Placeholder
        progress_data["total_books"] = total_books # Set total for progress

        while True:
            current_url = f"{url}&page={page}"
            print(f"Scraping page {page}...") # Simplified log
            if page == 1:
                 response = initial_response; soup = initial_soup
            else:
            ...
            for row in rows:
                ...
                if title_elem and author_elem:
                    ...
                    # Keep only essential fields needed for 3D render
                    books.append({ "title": title_elem.text.strip(), "image": high_res_image_url, "spine_color": spine_color })
                    progress_data["books_processed"] = len(books) # Update progress
            page += 1

        if not books and not progress_data.get("error"): progress_data["error"] = "No books found."
        progress_data["total_books"] = progress_data.get("books_processed", 0) # Match total to processed
...
# --- HTML Template (Includes JS fixes for Loader & Scroll) ---
...
<body>
    ...
    <script type="module">
        ...
        // --- Constants ---
        const BOOK_DEFAULTS = { HEIGHT: 2.0, WIDTH: 1.35, THICKNESS: 0.25 };
        const PAGE_COLOR = 0xf5f5dc;
        const BOOK_SPACING = -1; // User's overlap value from previous step
        const TARGET_ROTATION_X = THREE.MathUtils.degToRad(-90); // Final angle X
        const TARGET_ROTATION_Y = THREE.MathUtils.degToRad(88);  // Final angle Y
        ...
        const progressBarFill = document.getElementById('progress-fill'); const statusText = document.getElementById('status-text'); // Added loader elements
        ...
        let progressIntervalId = null; // Added interval ID
        ...
        // --- Handle Form Submit (with Loader Logic) ---
        async function handleUrlSubmit(event) {
             ...
             // --- Reset Loader and Start Polling ---
             ...
             try { const apiUrl = `/get_books?url=${encodeURIComponent(shelfUrl)}`; console.log("Fetching from:", apiUrl); const response = await fetch(apiUrl); let errorMsg = `HTTP error ${response.status}`; if (!response.ok) { try { const d=await response.json(); errorMsg = d.error||errorMsg; } catch (e) {} throw new Error(errorMsg); } const data = await response.json(); console.log("Received data:", data);
                // --- Stop Polling on Fetch Success/Error ---
        ...
        async function updateProgress() {
             try {
                 ...
                 if (!response.ok) { console.warn("Progress check failed:", response.status); return; }
                 const data = await response.json(); const percent = data.progress || 0;
                 if(progressBarFill){ progressBarFill.style.width = percent + '%'; progressBarFill.textContent = percent + '%'; }
                 if(statusText){ if (!data.complete && !data.error) { statusText.textContent = `Processing... (${data.books_processed}/${data.total_books})`; } }
                 if (data.complete || data.error) {
                     console.log("Progress poll end."); if (progressIntervalId) clearInterval(progressIntervalId); progressIntervalId = null;
                     if(progressBarFill){ progressBarFill.style.width = '100%'; progressBarFill.textContent = '100%';}
                     if(statusText && data.error){ statusText.textContent = `Error: ${data.error}`; }
                     else if(statusText && data.complete) { /* Let handleUrlSubmit show final message */ }
                 }
             } catch (error) { console.warn("Error fetching progress:", error); }
         }
        ...
        function populateScene() {
             ...
             console.log("Finished adding meshes."); loadingMessage.style.display = 'none'; // Hide loader AFTER books added
        ...
        function onWindowScroll() {
             // --- Define totalTravelDistance ---
             const totalTravelDistance = Math.max(0, bookData.length - 1) * (BOOK_DEFAULTS.HEIGHT + BOOK_SPACING);
             currentScrollY = window.scrollY; const maxScroll = document.documentElement.scrollHeight - window.innerHeight; const scrollRatio = maxScroll > 0 ? currentScrollY / maxScroll : 0;
             // Recalculate startY and initialGroupY based on current bookData
             const totalStackHeight = bookData.length * (BOOK_DEFAULTS.HEIGHT + BOOK_SPACING) - BOOK_SPACING;
...
def get_progress():
    ...
    # print(f"Progress: {processed}/{total} = {percent}%") # Optional server-side log
